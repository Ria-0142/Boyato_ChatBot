...
precedence = (
    ('nonassoc', 'LESS_THAN', 'GREATER_THAN', 'GREATER_EQUAL','LESS_EQUAL','EQUALS'),  # Comparison operators
...
def p_program_eval(p):
    ...
    if p[1] is not None:
        for statement in p[1]:
            evaluate(statement, global_scope)
...
def evaluate(node, scope):
    if isinstance(node, (int, float, str)):
        ...
        if op in ['+', '-', '*', '/','**','%']:
            ...
            scope.set('__return__', value)  # Store the return value in the function scope
            return value
            ...
            if function_definition:
                ...
                new_scope = Scope(parent=scope)  # Create a new scope with the current scope as parent
            ...
            if function_definition:
                ...
                for statement in statements:
                    evaluate(statement, new_scope)
                return_value = new_scope.get('__return__')  # Get the return value from the function scope
                scope.set(var_name, return_value)  # Assign the return value to the variable in the current scope
                return return_value
    else:
...
def execute_code(source_code):
    try:
        ...
        evaluate(result, global_scope)
        return "Output: Successful code execution"
